...
# FUNCTIONS ----
  ...
  viz_viral_load <- function(df, cntry, pd, src, rid,
    ...
    viz <- df %>% 
      ...
      #geom_text(aes(y = vls, label = percent(vls, 1)), vjust = "inward", hjust = "inward", color = genoa) +
...
# VIZ ----
  ...
  df_vlcs %>% 
    ...
    labs(x = "", y = "",
         title = glue::glue("{toupper(cntry)} - VIRAL LOAD TRENDS"),
         subtitle = glue::glue("<span style='color:{burnt_sienna}'>Coverage</span> & <span style='color:{genoa}'>Supression</span>"),
         caption = glue::glue("Source: {metadata_msd$source} - Created by OHA/SIEI | Ref. ID #{ref_id}")) +
    facet_wrap(~age) +
    si_style_nolines() +
    theme(plot.title = element_markdown(),
          plot.subtitle = element_markdown(),
